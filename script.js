...
function handleTouchMove(e) {
    ...
    // Only allow drag if long press was completed
    if (!isDraggingTouch) {
        // If user moves before long press completes, cancel it
...
function handleTouchEnd(e) {
    ...
    // Only perform swap if dragging was active (long press completed)
    if (isDraggingTouch) {
...
function initializeSettings() {
    ...
    const savedCustomPrimary = localStorage.getItem('customColorPrimary') || '#6366f1';
    const savedCustomSecondary = localStorage.getItem('customColorSecondary') || '#8b5cf6';
    ...
    applyCustomColorBtn.addEventListener('click', () => {
        ...
        // Calculate darker shade for primary-dark (darken by ~10%)
        const primaryDark = darkenColor(primary, 0.1);
        
        // Save to localStorage
        localStorage.setItem('customColorPrimary', primary);
        localStorage.setItem('customColorSecondary', secondary);
        localStorage.setItem('customColorPrimaryDark', primaryDark);
    ...
    // Load custom palette if it was saved
    if (currentPalette === 'custom') {
        const savedPrimary = localStorage.getItem('customColorPrimary');
        const savedSecondary = localStorage.getItem('customColorSecondary');
        const savedPrimaryDark = localStorage.getItem('customColorPrimaryDark');
        
        if (savedPrimary && savedSecondary && savedPrimaryDark) {
            document.body.style.setProperty('--custom-primary', savedPrimary);
            document.body.style.setProperty('--custom-primary-dark', savedPrimaryDark);
            document.body.style.setProperty('--custom-secondary', savedSecondary);
        }
    }
